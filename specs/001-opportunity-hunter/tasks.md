...
- [x] T009 [P] [US1] Unit test for extraction from list-page fixture `tests/fixtures/list_caba_deptos_venta_p1.html` in `tests/unit/test_extract.py`: RSC `initialItems` items (URL, `precio`, `localidad`, `provincia`, `coordenadas`, `direccion`) enriched with JSON-LD description/datePosted; JSON-LD `parse_ld_list_page` (URL, price, currency, beds, baths, area, barrio, region, agency per item)
...
- [x] T012 [P] [US1] Unit test for normalize/identity in `tests/unit/test_normalize.py`: source_listing_id parsed from URL, stable identity, barrio = property's `localidad` (never the agency's address), price → price_cents from `precio`, exact `coordenadas` → `lat`/`lng`, listing with no localidad bucketed under "unknown" within its region (zone-assignment coverage for SC-002)
...
- [x] T018 [P] [US1] Implement RSC + JSON-LD + HTML extraction in `src/property_hunter/ingest/extract.py` (`parse_rsc_list_payload` decodes the `self.__next_f.push([1,"..."])` chunks and reads `initialItems`; `parse_list_page` merges RSC items with JSON-LD description/datePosted, falling back to JSON-LD alone; parse `application/ld+json` blocks; extract ListingRecord fields per data-model.md)
- [x] T101 [P] [US1] Fix location/zone assignment: `normalize.py` derives barrio from the property's RSC `localidad` and stores the exact `coordenadas` as `lat`/`lng`; agency address is never used (JSON-LD `provider.address` is the agency office). Bug: run 1 mis-barrio'd 8367/8366→Villa Urquiza (Almagro/Mataderos) and 396→Belgrano (Flores), traced to `provider.address.addressLocality`. `db.py` adds `lat`/`lng` (DDL + idempotent migration via `_migrate`) and upsert persists them; contract `listing-v1.schema.json` gains `lat`/`lng`; regression tests pin coordinates per fixture (49→-34.6118561,-58.4245777).
...
- [x] T103 [P] [US1] Fix pagination: SSR `?pagina=N` pages re-render the same first 24 items (Mendoza run collected 23 of ~3.000). Collect page 1 via SSR as before, then paginate via the site's JSON API `POST /server/inmuebles/buscar` (body `{"filtros":…,"page":N,"limit":24}`; needs `Origin`/`Referer` headers; honors `page`, reports `total`; `limit` capped at 24). `extract.py` adds `parse_rsc_filters` (reads the embedded `"filters":{…}` search state off the SSR page — the exact `filtros` body, so no hardcoded region ids) and `api_items_to_cards` (maps API items to the canonical card shape); `inmoup.py` adds `make_fetcher`/`search_payload`/`SEARCH_ENDPOINT`; `client.py` adds `post_json`; `pipeline.py::_collect_via_api` pages until `total` reached or a page yields nothing, degrading to SSR-only on API failure. Tests: `parse_rsc_filters`, `api_items_to_cards`, and `test_collect_via_api_pagination` (monkeypatched fetch/API over fixtures, 50 listings across 2 API pages).
- [x] T104 [P] [US1] Fix detail URLs (dashboard links 404'd): the working URL uses the agency's **id-prefixed** slug (`17636-gold-group-argentina`), but `_card_url` used the bare `inmobiliaria.slug` from the API payload. Prefix `inmobiliaria.id` (guarding against already-prefixed slugs); `parse_list_page` already used the id-prefixed JSON-LD URL. Backfilled by re-collect (`upsert_listing` refreshes `source_url`).
- [x] T105 [P] [US1] Fix completion/delist bug: `ingest_items` completion signal counted *genuinely new* listings (no prior observation), which is 0 on every page of a re-run — pagination stopped after 1 API page and `mark_inactive_unseen` delisted 1.799 previously collected listings. `new_in_page` now counts ids newly seen this run (per-run dedupe), so re-runs paginate to `total` and nothing is delisted. Regression test `test_collect_rerun_keeps_everything_active`.
...
- [x] T102 [P] Add `ui` subcommand `src/property_hunter/ui.py` (stdlib-only read-only dashboard: `ThreadingHTTPServer` serving `/` + `/api/data`; listings with latest ask/prediction + filter/sort/search, detections with signal details + notification status, baselines, runs, model versions, notifications) + unit tests `tests/unit/test_ui.py` (data shape, missing DB, HTTP endpoints)
